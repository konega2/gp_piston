...
export function CreatePilotForm() {
  ...
  const updatePersonalInfo = (field: keyof PersonalInfo, value: string) => {
    ...
    if (field === 'kartNumber') {
      const normalizedKartNumber = value.replace(/\D/g, '');
      setPersonalInfo((prev) => ({ ...prev, [field]: normalizedKartNumber }));
      return;
    }
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const parsedKartNumber = personalInfo.kartNumber.trim() ? Number(personalInfo.kartNumber) : null;
      ...
      <FormSection title="INFORMACIÓN PERSONAL">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <TechnicalInput
            ...
            type="number"
            min="1"
            value={personalInfo.kartNumber}
            onChange={(value) => updatePersonalInfo('kartNumber', value)}
            placeholder="Ej: 105"
          />
          <TechnicalInput
            label="Teléfono"
...
function validatePersonalInfo(personalInfo: PersonalInfo): string | null {
  ...
  if (personalInfo.kartNumber.trim().length > 0) {
    const kartNumber = Number(personalInfo.kartNumber);
