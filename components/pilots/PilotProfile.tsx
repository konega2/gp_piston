...
export function PilotProfile({ pilot }: PilotProfileProps) {
  ...
  const saveChanges = () => {
    ...
    const normalizedDraft: PilotRecord = {
      ...
      numeroKart:
        typeof draftData.numeroKart === 'number' && Number.isInteger(draftData.numeroKart) && draftData.numeroKart > 0
          ? draftData.numeroKart
          : null
    };
    ...
    <div className="mx-auto max-w-6xl space-y-5 animate-fade-in">
              ...
              {isEditMode ? (
                <div className="grid gap-2 sm:grid-cols-2">
                  ...
                  <TechnicalInput
                    label="Nº Kart"
                    type="number"
                    value={draftData.numeroKart === null ? '' : String(draftData.numeroKart)}
                    onChange={(value) =>
                      setDraftData((prev) => ({
                        ...prev,
                        numeroKart: value.trim() === '' ? null : Number(value) > 0 ? Math.floor(Number(value)) : null
                      }))
                    }
                    className="max-w-[145px]"
                  />
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="rounded-md border border-white/15 bg-black/45 px-3 py-1.5 text-sm font-semibold uppercase tracking-[0.14em] text-white">
                    Kart {profileData.numeroKart ?? 'N/A'}
                  </span>
                </div>
              )}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <InfoPanel title="INFORMACIÓN PERSONAL" editMode={isEditMode}>
          ...
          <InfoItem
            label="Nº Kart"
            value={profileData.numeroKart ? String(profileData.numeroKart) : 'No asignado'}
            editMode={isEditMode}
            inputType="number"
            inputValue={draftData.numeroKart === null ? '' : String(draftData.numeroKart)}
            min={1}
            onInputChange={(value) =>
              setDraftData((prev) => ({
                ...prev,
...
function validatePilotDraft(pilot: PilotRecord): string | null {
  ...
  if (pilot.numeroKart !== null) {
    if (!Number.isInteger(pilot.numeroKart) || pilot.numeroKart <= 0) {
      return 'El número de kart debe ser un entero mayor que 0.';
    }
  }
