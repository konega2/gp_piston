...
export function PilotsProvider({ children }: { children: React.ReactNode }) {
  ...
  const maxPilots = runtimeConfig?.maxPilots ?? Number.MAX_SAFE_INTEGER;
  ...
  const addPilot = (pilot: NewPilotInput): PilotRecord => {
    ...
    const createdPilot: PilotRecord = {
      ...
      numeroKart:
        typeof pilot.numeroKart === 'number' && Number.isInteger(pilot.numeroKart) && pilot.numeroKart > 0
          ? pilot.numeroKart
          : null
    };
...
function validatePilotData(pilot: Pick<PilotRecord, 'nombre' | 'apellidos' | 'edad' | 'telefono' | 'peso' | 'numeroKart'>) {
  ...
  if (pilot.numeroKart !== null) {
    if (!Number.isInteger(pilot.numeroKart) || pilot.numeroKart <= 0) {
      throw new Error('El número de kart debe ser un entero mayor que 0.');
    }
  }
}
