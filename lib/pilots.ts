...
export async function createPilot(eventId: string, data: PilotInput): Promise<string> {
  ...
  await sql`
    INSERT INTO pilots (id, event_id, login_code, number, name, apellidos, edad, telefono, redes_sociales, peso, nivel, has_time_attack, kart_number)
    VALUES (
      ${id},
      ${eventId},
      ${loginCode},
      ...
      ${typeof data.kartNumber === 'number' && Number.isInteger(data.kartNumber) && data.kartNumber > 0 ? Math.floor(data.kartNumber) : null}
    );
  `;
...
export async function updatePilot(eventId: string, pilotId: string, data: PilotInput): Promise<void> {
  ...
  await sql`
    ...
    SET
      ...
      kart_number = ${typeof data.kartNumber === 'number' && Number.isInteger(data.kartNumber) && data.kartNumber > 0 ? Math.floor(data.kartNumber) : null}
